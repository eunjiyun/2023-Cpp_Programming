...
public:
	REV(typename Container::iterator it) : val(*it)
	{
	...
	// 추가된 생성자
	REV(const typename Container::value_type& value) : val(value)
	{
		std::reverse(val.begin(), val.end());
	}
};
...
//---------
{
	...
	cout << "첫 번째 단어 : " << v.front()<< endl;
	cout << "마지막 단어 : " << v.back() << endl;
	...
	//copy(v2.begin(), v2.end(), REV<std::vector<std::string>>(v2.begin()));
